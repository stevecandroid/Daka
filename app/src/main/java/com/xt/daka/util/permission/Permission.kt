...
class PermissionsWrapper {
     ...
     */
    class PermissionMgr private constructor(private val mActiviy: UtilActivity) {
        ...
        companion object {
            fun with(mActiviy: UtilActivity): PermissionMgr {
        ...
        fun onRequestPermissionsResult(requestCode: Int, p: Array<out String>, grantResults: IntArray) {
            ...
            if (t != grantResults.size) {
                if (wrapper.onSomeDenied != null) {
                    ...
                    if (defaultConfig) {
                        mActiviy.alert("请到本应用权限页面下,允许所有权限以保证应用的使用") {
                            yesButton {
